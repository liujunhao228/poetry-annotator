...
class ResponseParsingCore:
    """
    响应解析核心处理器类，负责处理响应解析的核心逻辑。
    """
    ...
    def __init__(self, project_root=None):
        ...
        # 获取所有已注册的响应解析插件
        self.response_parsing_plugins = self.plugin_manager.get_plugins_by_type(PluginType.RESPONSE_PARSING)
        self.response_parsing_plugins = [
            plugin for plugin in self.response_parsing_plugins
            if isinstance(plugin, ResponseParsingPlugin)
        ]

        logger.info(f"找到 {len(self.response_parsing_plugins)} 个响应解析插件")
    ...
    def parse_response(self, text: str) -> List[Dict[str, Any]]:
        ...
        """
        # 如果有插件，使用第一个插件处理
        if self.response_parsing_plugins:
            try:
                return self.response_parsing_plugins[0].parse_response(text)
            except Exception as e:
                logger.error(f"插件处理响应解析时出错: {e}")
    ...
    def _validate_annotation_list_content(self, data: list) -> List[Dict[str, Any]]:
        ...
        for item in data:
            if not isinstance(item, dict):
                raise ValueError("列表中的每个元素都必须是字典类型")
