...
[
    ...
    def _build_user_prompt(self, poem_data: PoemData) -> str:
        """
        构建用户提示词
        """
        # 生成带ID的句子列表
...
--- 输出 ---"""
    ...
    def build_prompts(self, poem_data: PoemData, emotion_schema: EmotionSchema,
                      model_config_name: str) -> Tuple[str, str]:
        """
        构建系统提示词和用户提示词
        ...
        """
        self.logger.info(f"开始为诗词 '{poem_data.title}' (ID: {poem_data.id}) 构建提示词...")
