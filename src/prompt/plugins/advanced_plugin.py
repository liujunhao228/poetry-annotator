...
class AdvancedPromptBuilderPlugin(PromptBuilderPlugin):
    """高级Prompt构建插件，支持更复杂的Prompt构建逻辑"""
    ...
    def get_description(self) -> str:
        return "高级Prompt构建器，支持更复杂的Prompt构建逻辑"
    ...
    def build_prompts(self, poem_data: PoemData, emotion_schema: EmotionSchema, 
        ...
        if 'gpt' in model_name.lower():
            system_prompt = self._build_gpt_system_prompt(emotion_schema.text)
        elif 'qwen' in model_name.lower():
            system_prompt = self._build_qwen_system_prompt(emotion_schema.text)
        else:
...
--- 输出 ---"""
    ...
    def _build_gpt_system_prompt(self, emotion_schema: str) -> str:
        """构建系统提示词"""
        return f"""# 角色
你是一位精通中国古典文学和情感分析的专家，特别擅长分析GPT模型的输出格式。
...
[
    ...
    def _build_qwen_system_prompt(self, emotion_schema: str) -> str:
        """为Qwen模型构建系统提示词"""
        return f"""<|im_start|>system
# 角色
你是一位精通中国古典文学和情感分析的专家，特别擅长分析通义千问模型的输出格式。
