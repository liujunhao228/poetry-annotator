...
class PoemClassificationPluginAdapter(PoemClassificationPlugin):
    """诗词预处理分类插件适配器"""
    ...
    def __init__(self, plugin_config: PluginConfig):
        ...
        try:
            ...
            logger.info(f"成功创建诗词预处理分类插件实例: {module_name}.{class_name}")
        except Exception as e:
            logger.error(f"创建诗词预处理分类插件实例失败: {e}")
            raise
    
    def get_name(self) -> str:
        """获取插件名称"""
        return self.plugin_config.settings.get('name', 'Unknown Poem Classification Plugin')
    
    def get_description(self) -> str:
        """获取插件描述"""
        return self.plugin_config.settings.get('description', '诗词预处理分类插件')
