# gui/script_panels/export_poem_annotations_panel.py
# Implements the ExportPoemAnnotationsPanel for the export_poem_annotations.py script.

import logging
import gettext
from PyQt5.QtWidgets import (
    QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QTextEdit, QFormLayout, QComboBox, QMessageBox, QFileDialog
)
from ..script_panel_base import ScriptPanelBase

_ = gettext.gettext

class ExportPoemAnnotationsPanel(ScriptPanelBase):
    """
    GUI panel for the export_poem_annotations.py script.
    Allows users to specify database name, output file, and format, then run the script.
    """
    def __init__(self, parent=None):
        super().__init__("export_poem_annotations.py", parent)

    def setup_ui(self):
        """
        Builds the specific UI for the Export Poem Annotations script.
        """
        ...
        self.format_combo.addItems([_("json"), _("csv"), _("markdown")]) # Assuming these are supported formats
        form_layout.addRow(QLabel(_("Output Format:")), self.format_combo)

        # Run Button
        self.run_button = QPushButton(_("Export Annotations"))
    ...
    def validate_input(self, inputs: dict) -> bool:
        ...
        if not inputs.get("format"):
            self.show_message(_("Validation Error"), _("Output Format cannot be empty."), QMessageBox.Warning)
            return False
        return True
