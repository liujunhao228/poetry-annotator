...
def get_random_poem_ids(db_name, sample_size=1, exclude_annotated=False, model_identifier=None, active_only=False):
    ...
    try:
        ...
        try:
            ...
            # 如果需要排除已标注的诗词
            completed_ids_list = []
            if exclude_annotated and annotation_cursor:
                if model_identifier:
                    # 查询指定模型已成功标注的诗词ID
                    completed_query = """
                    ...
                    """
                    annotation_cursor.execute(completed_query, (model_identifier,))
                    rows = annotation_cursor.fetchall()
                    ...
                    """
                    annotation_cursor.execute(completed_query)
                    rows = annotation_cursor.fetchall()
                    completed_ids = {row[0] for row in rows}
            ...
            # 获取最大ID和符合条件的记录总数
            raw_data_cursor.execute("SELECT MAX(id) FROM poems")
            max_id_result = raw_data_cursor.fetchone()
            max_id = max_id_result[0] if max_id_result else 0
            
            count_query = f"SELECT COUNT(id) FROM poems {where_clause}"
            raw_data_cursor.execute(count_query, tuple(query_params))
            total_records_result = raw_data_cursor.fetchone()
            ...
            if sample_size > total_records / 2: 
                ...
                raw_data_cursor.execute(select_query, tuple(query_params))
                all_ids = [row[0] for row in raw_data_cursor.fetchall()]
                ...
                while len(selected_ids) < sample_size:
                    ...
                    raw_data_cursor.execute(query, tuple(query_params_local))
                    # 只添加需要的数量，避免超过sample_size
                    fetched_ids = [row[0] for row in raw_data_cursor.fetchall()]
