...
"""
诗词标注任务分发工具
用于从文件读取诗词 ID 并分发标注任务，支持多模型、高并发。
支持传入单个列表文件或包含多个列表文件的目录，并可将目录内文件与模型顺序匹配。

【新日志系统适配说明】
此版本已适配新的分层日志系统 (控制台 INFO, 文件 DEBUG)。
- 增加了 `--console-log-level`, `--file-log-level`, `--enable-file-log` 命令行参数。
- 这些参数可以覆盖 config.ini 中的设置，方便从命令行或 GUI 进行精细化控制。
- 脚本内的日志记录遵循新规范：
...
# 全局 Project 实例，用于获取 Annotator
_project_instance: Optional[Project] = None


def get_db_path_by_name(db_name, config_mgr: ConfigManager):
    """
    根据数据库名称获取数据库路径
    """
...
def get_project_instance() -> Project:
    ...
    if _project_instance is None:
        ...
        if projects_dir.exists() and projects_dir.is_dir():
            # 查找 projects 目录下的第一个包含 config.ini 的子目录
            for item in projects_dir.iterdir():
                if item.is_dir() and (item / "config.ini").exists():
                    _project_instance = Project(
                        project_name=item.name,
                        project_root_dir=projects_dir,
                        global_config_path=project_root / "config.ini"
                    )
                    logger.info(f"已加载项目：{item.name}")
                    break
        
        # 如果 projects 目录下没有找到，则回退到直接在根目录下查找
        if _project_instance is None:
            for item in project_root.iterdir():
                if item.is_dir() and (item / "config.ini").exists() and item.name != "projects":
                    _project_instance = Project(
                        project_name=item.name,
...
class ProgressManager:
    ...
    def __init__(self, model_name: str, id_file_path: str):
        ...
        logger.debug(f"使用进度文件：{self.state_file} (基于 ID 文件：{id_file_path})")
...
def read_poem_ids_in_chunks(file_path: str, chunk_size: int):
    ...
    path = Path(file_path)
    if not path.exists():
        raise FileNotFoundError(f"ID 文件不存在：{file_path}")
...
def process_chunk(args: Tuple[str, List[int], bool, Project]) -> Dict[str, Any]:
    """
    【内层并发单元】工作线程执行的函数，处理一个批次的 ID。
    """
    model_name, poem_ids_chunk, force_rerun, project_ctx = args
    thread_ident = threading.get_ident()
    logger.debug(f"线程 {thread_ident} 开始为模型 '{model_name}' 处理 {len(poem_ids_chunk)} 个 ID 的批次。")
    try:
        # 使用项目上下文获取 Annotator 实例
        annotator = project_ctx.get_annotator(model_name)
...
def run_annotation_for_model(model: str, id_file: str, force_rerun: bool, chunk_size: int, fresh_start: bool, max_workers: int, project_ctx: Project):
    """
    【外层并发单元】为单个指定模型和单个 ID 文件运行完整的并行标注流程。
    """
    # 设置环境变量，供日志系统使用
    import os
    ...
    # 初始化批次日志记录器
    logger.info(f"开始新的批次任务 - 模型：{model}, ID 文件：{Path(id_file).name}")

    logger.info("=" * 60)
    logger.info(f"启动任务流水线 -> 模型：[{model}], ID 文件：[{Path(id_file).name}]")
    logger.info(f"任务将使用 {max_workers} 个内部工作线程处理数据块。")
    logger.debug(f"完整 ID 文件路径：[{id_file}]")
    logger.info("=" * 60)
    ...
    try:
        ...
        if not id_chunks:
            logger.warning(f"ID 文件 '{id_file}' 为空或不包含有效 ID。跳过此任务。")
            return
        ...
        # 传递 project_ctx 给每个任务
        task_args = [(model, chunk, force_rerun, project_ctx) for chunk in chunks_to_process]
        ...
        logger.error(f"任务 [{model}]-[{Path(id_file).name}] 失败：读取 ID 文件时出错：{e}")
        return
    except Exception as e:
    ...
    logger.info("-" * 60)
    logger.info(f"任务流水线完成 -> 模型：[{model}], ID 文件：[{Path(id_file).name}]")
    logger.info(f"本次运行耗时：{total_duration_this_run:.2f} 秒")
    logger.info(f"累计总耗时：{final_total_duration:.2f} 秒")
    logger.info(f"总处理 ID 数：{total_processed_count}")
...
@click.option('--id-file', '-f', type=click.Path(exists=True, dir_okay=False, resolve_path=True),
              help="包含诗词 ID 的文本文件路径 (与 --id-dir 互斥)。如果指定，该文件将被所有选定模型使用。")
@click.option('--id-dir', '-d', type=click.Path(exists=True, file_okay=False, resolve_path=True),
              help="包含多个诗词 ID 文件的目录路径 (与 --id-file 互斥)。")
...
def cli(model, all_models, id_file, id_dir, force_rerun, chunk_size, fresh_start, db, console_log_level, file_log_level, enable_file_log):
    """
    【主控制函数】从文件分批读取诗词 ID 并以多模型并发方式分发标注任务。
    """
    ...
    # 初始化 Project 实例
    try:
        project_ctx = get_project_instance()
    ...
    # 先初始化 project_ctx 以获取 LLMFactory 实例
    # 从 project_ctx 获取 LLMFactory 实例
    llm_factory_instance = project_ctx.llm_factory
    available_models = llm_factory_instance.list_configured_models().keys()
    ...
    # --- 执行健康检查 ---
    # 使用已获取的 LLMFactory 实例和 ConfigManager 实例传递给健康检查器
    config_manager_instance = project_ctx.config_manager
    if not asyncio.run(health_checker.run_all_checks(
        models_to_run,
        llm_factory_instance,
        config_manager_instance
    )):
        logger.critical("任务因健康检查失败而中止。")
        return

    # --- 2. 确定 ID 文件 (s) 和模型 - 文件映射 ---
    ...
    if id_file:
        ...
        if len(models_to_run) != len(id_files_in_dir):
            ...
            logger.error(f"模型：{models_to_run}\nID 文件：{[Path(f).name for f in id_files_in_dir]}")
            return
