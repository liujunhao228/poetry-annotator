...
class LLMFactory:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        try:
            ...
            self.logger.warning(f"加载 LLM 配置失败，将使用默认熔断器参数：{e}")
    ...
    def get_llm_service(self, config_name: str) -> BaseLLMService:
        ...
        try:
            ...
            model_config = self.config_manager.get_model_config(config_name)
            ...
            self.logger.error(f"创建 LLM 服务失败：{e}")
            raise ValueError(f"创建 LLM 服务实例 '{config_name}' 失败：{e}") from e

    def list_configured_models(self) -> Dict[str, Dict[str, str]]:
        """列出在 config.ini 中所有已配置的模型"""
        configured_models = {}
        config_names = self.config_manager.list_model_configs()
    ...
    def validate_config_name(self, config_name: str) -> bool:
        """验证模型配置名称是否有效"""
        return config_name in self.config_manager.list_model_configs()
