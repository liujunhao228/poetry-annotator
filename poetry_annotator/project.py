...
class Project:
    ...
    def __init__(self, project_name: str, project_root_dir: Path, global_config_path: Path):
        ...
        # 兼容两种配置文件名：project_config.ini 或 config.ini
        self.project_config_file_path = self.root_path / "project_config.ini"
        if not self.project_config_file_path.exists():
            fallback_config = self.root_path / "config.ini"
            if fallback_config.exists():
                self.project_config_file_path = fallback_config
                logger.debug(f"使用备用配置文件：{self.project_config_file_path}")
    ...
    def ensure_project_dirs(self):
        ...
        logger.debug(f"项目 '{self.name}' 的数据目录和日志目录已确认存在。")
